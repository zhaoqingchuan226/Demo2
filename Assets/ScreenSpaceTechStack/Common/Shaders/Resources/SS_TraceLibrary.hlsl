    ...
    */
    ...
    {
        //RayCastSize=1 / _SSGi_RayCastSize.xy，RayCastSize在一般情况下就是1/相机宽高
        float SamplerSize = GetMarchSize(rayStart.xy, rayStart.xy + rayDir.xy, RayCastSize);//保证了第一次步进不会跨出一个像素
        float3 samplePos = rayStart + rayDir * (SamplerSize);
        int level = HiZ_Start_Level; float mask = 0.0;
        
        UNITY_LOOP
        for (int i = 0; i < NumSteps; i++)//NumSteps=参数面板HiZ_RaySteps，表示raymarching的最多次数
        {
            float2 cellCount = RayCastSize * exp2(level + 1.0);//此level最多允许跨出去的像素
            float newSamplerSize = GetMarchSize(samplePos.xy, samplePos.xy + rayDir.xy, cellCount);//保证每一步的跨度小于此level的最大容忍值
            ...
            UNITY_FLATTEN//全部执行，适合if else都会存在的情况
            if (sampleMinDepth < newSamplePos.z)//跨一步试试，表示没碰到东西，还可以跨大点
            {
                level = min(HiZ_Max_Level, level + 1.0);
                samplePos = newSamplePos;
            } 
            else//表示碰到了，那就回去，这次跨小一点
            {
                level--;
            }

            UNITY_BRANCH//执行if，适合仅if存在的情况
            if (level < HiZ_Stop_Level) //如果已经小于最小进度了，那就停止吧
            {
                float delta = (-LinearEyeDepth(sampleMinDepth)) - (-LinearEyeDepth(samplePos.z));//嵌入的距离
                mask = delta <= thickness && i > 0.0;
                //i > 0.0 如果是第一次就嵌入了，那不算，但是HiZ_Stop_Level很小，这是不可能的，所以这句代码是废代码
                //
        ...
        return float4(samplePos, mask);//如果一直找不到，是会返回天空盒颜色的
