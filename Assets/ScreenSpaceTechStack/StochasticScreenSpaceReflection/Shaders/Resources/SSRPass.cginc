...
half4 CombineReflectionColor(PixelInput i) : SV_Target 
{
	...
	half4 CubemapColor = tex2D(_CameraReflectionsTexture, uv) ;
	...
	//这是后面加上去的，为了消除背景的奇怪的三角形
	half Mask=step(0.0001,SceneDepth);
	SSRMask*=Mask;
	
	// return  (CubemapColor * (1 - SSRMask));
	// return SSRColor*SSRMask*PreintegratedGF;
	
	// return SpecularColor*half4(SSRColor.rgb,1)*SSRMask+SceneColor*(1-SSRMask);//原来返回这一句

	// return half4(lerp(SceneColor.rgb,SSRColor.rgb*SSRMask*SpecularColor,SSRMask),1);
