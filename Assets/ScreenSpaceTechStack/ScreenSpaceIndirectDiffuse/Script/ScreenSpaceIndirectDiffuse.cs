...
public class ScreenSpaceIndirectDiffuse : MonoBehaviour
{
    ...
    private enum DebugPass//是否只显示间接光
    {
        Combine = 5,
        IndirectColor = 6
    };
    ...
    private Matrix4x4 SSGi_ProjectionMatrix;//现在的
    private Matrix4x4 SSGi_ViewProjectionMatrix;//过去的
    ...
    void OnPreRender()//每次渲染前
    {
        RandomSampler = GenerateRandomOffset();//生成二维随机数
        SSR_UpdateVariable();//更新传入shader的参数
    ...
    private float GetHaltonValue(int index, int radix)//随机数核心算法
    {
        float result = 0f;
    ...
    private void SSR_UpdateVariable()
    {
        ...
        if (CameraSize != CurrentCameraSize)
        {
            ...
            SSGi_HierarchicalDepth_RT.useMipMap = true;//手动添加depth的mip
            SSGi_HierarchicalDepth_RT.autoGenerateMips = false;//关闭自动mip
        ...
        SSGi_Material.SetVector(SSGi_Jitter_ID, new Vector4((float)CameraSize.x / 1024, (float)CameraSize.y / 1024, RandomSampler.x, RandomSampler.y));//xy尺寸
    ...
    private void RenderScreenSpaceIndirectDiffuse()
    {
        ...
        SSGi_Buffer.Blit(BuiltinRenderTextureType.ResolvedDepth, SSGi_HierarchicalDepth_RT);//src是从延迟中解析深度缓冲区，原始深度图
        for (int i = 1; i < HiZ_MaxLevel; ++i)
        {
            ...
            SSGi_Buffer.DrawMesh(GraphicsUtility.mesh, Matrix4x4.identity, SSGi_Material, 0, RenderPass_HiZ_Depth);//一个cmd的DrawMesh要和SetRenderTarget一起用，先设置RT目标，再draw片片
            SSGi_Buffer.CopyTexture(SSGi_HierarchicalDepth_BackUp_RT, 0, i, SSGi_HierarchicalDepth_RT, 0, i);//到这一步，拥有正确mipmap的深度图就做好了
        }
        ...
        SSGi_Buffer.CopyTexture(BuiltinRenderTextureType.CameraTarget, SSGi_SceneColor_RT);//当前帧的颜色缓冲区


        //////RayCasting//////
        SSGi_Buffer.SetGlobalTexture(SSGi_Trace_ID, SSGi_TraceMask_RT);
        SSGi_Buffer.BlitSRT(SSGi_TraceMask_RT, SSGi_Material, RenderPass_HiZ3D_MultiSpp);//光线追踪

        if (Denoise)
        {
            //////Temporal filter//////
            //时间滤波进行的地方
            SSGi_Buffer.SetGlobalTexture(SSGi_TemporalCurr_ID, SSGi_TemporalCurr_RT);//这是上一帧的TAA 前RT
            SSGi_Buffer.BlitSRT(SSGi_TemporalCurr_RT, SSGi_Material, RenderPass_Temporalfilter);//TAA，用的pre是上上帧的（已空间），用到cur是上一帧的，之所以这么做是要等空间滤波完成
            SSGi_Buffer.SetGlobalTexture(SSGi_TemporalPrev_ID, SSGi_TemporalPrev_RT);//这是上一帧的TAA 后RT
            SSGi_Buffer.CopyTexture(SSGi_TemporalCurr_RT, SSGi_TemporalPrev_RT);

            //之后用的cur和pre都是上一帧的，无区别
